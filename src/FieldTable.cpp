...
FieldTable::FieldTable(ClassFile *classFile) {
    for (int i = 0; i < classFile->fields_count; ++i) {
        ...
        field->name = std::string((char *)((CONSTANT_Utf8_info*) classFile->constant_pool[fieldInfo.name_index - 1].info)->bytes);
        field->descriptor = std::string((char *)((CONSTANT_Utf8_info*) classFile->constant_pool[fieldInfo.descriptor_index - 1].info)->bytes);
